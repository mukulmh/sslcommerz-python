...
def Checkout(request):
    transaction_id = ''.join(secrets.choice(string.ascii_uppercase + string.digits) for i in range(18))
    amount = 100

    mypayment = SSLCSession(sslc_is_sandbox=True, sslc_store_id='devel634786da0d226', sslc_store_pass='devel634786da0d226@ssl')
    ...
    mypayment.set_product_integration(tran_id=transaction_id, total_amount=amount, currency='BDT', product_category='clothing', product_name='demo-product', num_of_item=2, shipping_method='YES', product_profile='None')
    ...
    if (response_data['status']== 'SUCCESS'):
        Transactions.objects.create(tran_id = transaction_id, amount = amount, status='PENDING')
